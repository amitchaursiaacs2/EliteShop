        ...
        <>
            <ClickAwayListener onClickAway={() => setOpen(false)}>
                <div className="selectDropWrapper cursor position-relative">
                    {icon}
                    <span className="openSelect" onClick={openDrop}>{selectedItem.length > 10 ? selectedItem.substr(0, 10) + "..." : selectedItem}<IoIosArrowDown className="arrowDrop" /> </span>
                    {
                        open === true &&
                        <div className="selectDrops">
                            ...
                            <ul className="searctResults">
                                <li key={0} onClick={() => closeDrop(0, placeholder)} className={`${selected === placeholder ? 'active' : ''}`}>{placeholder}</li>
                                {
                                    listData.map((item, index) => {
                                        return (
                                            <li key={index} onClick={() => closeDrop(index, item)} className={`${selected === index ? 'active' : ''}`}>{item}</li>
                                        )
                                    })
                                }
                            </ul>
